...
<div id="div_right_frame">
	<div id="div_position_nav">当前位置: 游戏日志 >> 游戏内活跃事件日志</div>
		...
		类型：<input id="checkallType"   type="checkbox"  onclick="selectAllType()"/> 全选&nbsp;
				<c:if test="${not empty eventTypeMap2}">
					<c:forEach items="${eventTypeMap2}" var="typeMap" varStatus="status">
						...
						<c:choose>
						<c:when test="${ fn:contains(eventTypesList,typeMap.key) }"><input type="checkbox" checked='checked' name="eventTypes"  value="${typeMap.key}" />${typeMap.value}&nbsp;</c:when>
						<c:otherwise><input type="checkbox" id="ck"  name="eventTypes" value="${typeMap.key}" />${typeMap.value}&nbsp;</c:otherwise>
						</c:choose>
						<c:choose>
		...
		<c:choose>
			<c:when test="${not empty list.records }">
				...
				<tr class="tr_list_data">	
				 	...
				 	<td class="center"> ${item.failTimes}</td> 	
				 	<td class="center"> ${item.succTimes}</td>										
