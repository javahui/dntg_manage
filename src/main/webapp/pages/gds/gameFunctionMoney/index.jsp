...
<div id="div_right_frame">
	<div id="div_position_nav"><span>当前位置</span>：游戏数据统计 >> 游戏内活跃事件货币变化</div>
	...
	<table class="tbl_list">
		<tr class="tr_list_title">
			<td width="10%"> 时间 </td>
			<td width="10%"> 总计事件进行次数 </td>
			<td width="10%"> 总计获取元宝 </td>
			<td width="10%"> 总计充值元宝 </td>
			<td width="10%"> 总计消耗元宝 </td>
			<td width="10%"> 总计获取绑定元宝 </td>
			<td width="10%"> 总计消耗绑定元宝 </td>
			<td width="10%"> 总计获取游戏币 </td>
			<td width="10%"> 总计消耗游戏币 </td>
		</tr>
		<c:choose>
			<c:when test="${not empty list.records }">
				<c:forEach items="${list.records}" var="gameFunctionMoney" varStatus="status">
				<tr id="recordTr${status.index}" class="tr_list_data">
					 <td class="center">  <fmt:formatDate value="${gameFunctionMoney.logDay}" pattern="yyyy-MM-dd"/></td> 			
					 <td class="center"> ${gameFunctionMoney.totalTimes}</td>
					 <td class="center"> ${gameFunctionMoney.gainYb}</td>
					 <td class="center"> ${gameFunctionMoney.rechargeYb}</td>
					 <td class="center"> ${gameFunctionMoney.consumeYb}</td>
					 <td class="center"> ${gameFunctionMoney.gainBindYb}</td>
					 <td class="center"> ${gameFunctionMoney.consumeBindYb}</td>
					 <td class="center"> ${gameFunctionMoney.gainYxb}</td>
					 <td class="center"> ${gameFunctionMoney.consumeYxb}</td>
				</tr>
				</c:forEach>
