...
<div id="div_right_frame">
	<div id="div_position_nav">当前位置：游戏数据统计 >> 游戏币消耗</div>
	...
	<table class="tbl_list">
		<tr class="tr_list_title">
			<td width="15%"> 事件 </td>
			<td width="15%"> 游戏币消耗总量 </td>
			<td width="15%"> 消耗次数 </td>
			<td width="25%"> 时间 </td>
		</tr>
		<c:choose>
			<c:when test="${not empty list.records }">
				<c:forEach items="${list.records}" var="yxbConsume" varStatus="status">
				<tr id="recordTr${status.index}" class="tr_list_data">
					 <td class="center"> ${eventTypeMap[yxbConsume.eventType]}</td> 					
					 <td class="center"> ${yxbConsume.yxb}</td>
					 <td class="center"> ${yxbConsume.times}</td> 					
					 <td class="center"><fmt:formatDate value="${yxbConsume.logDay}" pattern="yyyy-MM-dd"/></td> 					
				</tr>
				</c:forEach>
