...
<div id="div_right_frame">
	<div id="div_position_nav"><span>当前位置</span>：游戏数据统计 >> 创建时间人数统计</div>
		...
		<c:choose>
			<c:when test="${not empty list.records }">
				...
				<c:if test="${status.index==0 }">
					<tr id="recordTr${modelMap.allCount}" class="tr_list_data">
						<td class="right" >总人数</td>
						<td class="right" >${modelMap.allCount} </td>
						<td class="right">100.0000</td>
						<td></td>
					</tr>
				</c:if>
				<tr id="recordTr${status.index}" class="tr_list_data">
					<td class="right">${modelMap.createTime}</td>
					<td class="right">${modelMap.cnt}</td>
					<td class="right">${modelMap.ct_cnt} </td>
					<td></td>
				</tr>
				</c:forEach>
