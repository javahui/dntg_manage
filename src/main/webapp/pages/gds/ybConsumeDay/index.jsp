...
<div id="div_right_frame">
	<div id="div_position_nav"><span>当前位置</span>：游戏数据统计 >> 元宝消耗</div>
	...
	<table class="tbl_list">
		<tr class="tr_list_title">
			<td width="15%"> 事件 </td>
			<td width="15%"> 元宝消耗总量 </td>
			<td width="15%"> 次数 </td>
			<td width="25%"> 时间 </td>
		</tr>
		<c:choose>
			<c:when test="${not empty list.records }">
				<c:forEach items="${list.records}" var="ybConsume" varStatus="status">
				<tr id="recordTr${status.index}" class="tr_list_data">
					 <td class="center"> ${eventTypeMap[ybConsume.eventType]}</td> 					
					 <td class="center"> ${ybConsume.yb}</td> 								
					 <td class="center"> ${ybConsume.times}</td> 					
					 <td class="center"> <fmt:formatDate value="${ybConsume.logDay}" pattern="yyyy-MM-dd"/></td>
				</tr>
				</c:forEach>
