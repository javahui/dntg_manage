...
<div id="div_right_frame">
	<div id="div_position_nav"><span>当前位置</span>：游戏数据统计 >> 元宝获取</div>
	...
	<table class="tbl_list">
		<tr class="tr_list_title">
			<td width="15%"> 事件 </td>
			<td width="15%"> 获取元宝总量 </td>
			<td width="15%"> 获取次数 </td>
			<td width="25%"> 时间 </td>
		</tr>
		<c:choose>
			<c:when test="${not empty list.records }">
				<c:forEach items="${list.records}" var="ybIncr" varStatus="status">
				<tr id="recordTr${status.index}" class="tr_list_data">
					 <td class="center"> ${eventTypeMap[ybIncr.eventType]}</td> 					
					 <td class="center"> ${ybIncr.yb}</td>					
					 <td class="center"> ${ybIncr.times}</td> 					
					 <td class="center"><fmt:formatDate value="${ybIncr.logDay}" pattern="yyyy-MM-dd"/></td> 					
				</tr>
				</c:forEach>
