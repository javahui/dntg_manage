...
<div id="div_right_frame">
	<div id="div_position_nav"><span>当前位置</span>：游戏数据统计 >> 道具消耗</div>
	...
	<table class="tbl_list">
		<tr class="tr_list_title">
			...
			<td width="15%"> 消耗物品名称 </td>
			<td width="15%"> 消耗总数量 </td>
			<td width="25%"> 时间 </td>
		</tr>
		<c:choose>
			<c:when test="${not empty list.records }">
				<c:forEach items="${list.records}" var="propConsume" varStatus="status">
				<tr id="recordTr${status.index}" class="tr_list_data">				
										 <td class="center"> ${eventTypeMap[propConsume.eventType]}</td> 					
										 <td class="center"> ${propConsume.times}</td>							
										 <td class="center"> ${propConsume.goodsName}</td> 					
										 <td class="center"> ${propConsume.count}</td> 					
										 <td class="center">  <fmt:formatDate value="${propConsume.logDay}" pattern="yyyy-MM-dd"/></td> 		
				</tr>
				</c:forEach>
