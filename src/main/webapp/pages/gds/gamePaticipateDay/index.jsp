...
<div id="div_right_frame">
	<div id="div_position_nav"><span>当前位置</span>：游戏数据统计 >> 游戏内活跃事件每日统计</div>
	...
	<table class="tbl_list">
		...
		<c:choose>
			<c:when test="${not empty list.records }">
				<c:forEach items="${list.records}" var="gamePaticipate" varStatus="status">
				<tr id="recordTr${status.index}" class="tr_list_data">
					 <td class="center"> ${eventTypeMap[gamePaticipate.event_type]} </td> 					
					 <td class=" center"> ${gamePaticipate.fail_times}</td> 					
					 <td class="center"> ${gamePaticipate.succ_times}</td> 					
					 <td class="center"> <fmt:formatDate value="${gamePaticipate.log_day}" pattern="yyyy-MM-dd"/></td>
				</tr>
				</c:forEach>
