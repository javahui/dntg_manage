...
/**
 * dbUtils根据当前serverId连接日志数据库
 * @author donghui
 */
public class LogDbUtils {  
	private static final Logger log = LoggerFactory.getLogger(LogDbUtils.class);
	
	
    public static Connection getConnection() {
    	String serverId = LookupContext.getCurrentServerId();
		String gameDbUrl = (String)ToolDbUtils.selectObject("SELECT log_db_url FROM game_server WHERE server_id = '" + serverId+"'");
    ...
    public static Object selectObject(String sql) {
    	if (LogDbUtils.selectList(sql).size() > 0) {
    		Map map = LogDbUtils.selectList(sql).get(0);
    ...
    public static Map selectMap(String sql) {
    	List<Map> list = LogDbUtils.selectList(sql);
