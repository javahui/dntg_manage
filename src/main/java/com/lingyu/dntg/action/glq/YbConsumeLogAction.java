...
import com.lingyu.dntg.dao.glq.ConsumeYbDetailDao;
 ...
 */
@Controller
@RequestMapping("ybConsumeLog")
public class YbConsumeLogAction extends AbstractAction{
	@Resource private ConsumeYbDetailDao consumeYbDetailDao;
	...
	public String index(String userName, String startTime,
		...
		if (StringUtils.isNotBlank(userName) || StringUtils.isNotBlank(startTime) || StringUtils.isNotBlank(endTime)||StringUtils.isNotBlank(eventTypes)) {
		    ...
		    modelMap.put("list",logDaoTemplate.paging("ybConsume.index", paramMap));
	...
	public String indexDetail(String month, HttpServletRequest request, ModelMap modelMap) {
		if (StringUtils.isNotBlank(month)) {
			Map paramMap = (Map) request.getAttribute("paramMap");
			modelMap.put("list", consumeYbDetailDao.selectByMonth(month, paramMap));
		    modelMap.put("eventTypesList",paramMap.get("eventTypesList"));
		}
		modelMap.put("eventTypeMap2",StatisticsTaskGameEventTypeMapper.getRelateEventTypes(StatisticsTaskName.CONSUME_YB));
		modelMap.put("eventTypeMap",GameLogEventType.getEventTypeMap());
	...
	public ResponseEntity<byte[]>  xls(HttpServletRequest request, ModelMap modelMap){
		Map paramMap = (Map)request.getAttribute("paramMap");
		List<Map> dataList = logDaoTemplate.selectList("ybConsume.logExcelIndex", paramMap);
		...
		String[] titlesArray = new String[]{"角色名","事件","元宝消耗总量","消耗后角色元宝","次数","时间"};
		return super.exportXls(dataList, "元宝消耗日志", titlesArray);
	}
}
