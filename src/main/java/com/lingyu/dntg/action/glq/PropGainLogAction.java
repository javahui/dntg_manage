...
import com.lingyu.dntg.dao.glq.GainPropDetailDao;
 ...
 */
@Controller
@RequestMapping("propGainLog")
public class PropGainLogAction  extends AbstractAction{
	@Resource private GainPropDetailDao gainPropDetailDao;
	...
	public String index(String userName, String goodName,String startTime,
		...
		if (StringUtils.isNotBlank(userName)||StringUtils.isNotBlank(goodName)
		    ...
		    modelMap.put("list",logDaoTemplate.paging("propGain.index", paramMap));
	...
	public String indexDetail(String month, HttpServletRequest request, ModelMap modelMap) {
		if (StringUtils.isNotBlank(month)) {
			Map paramMap = (Map) request.getAttribute("paramMap");
			modelMap.put("list", gainPropDetailDao.selectByMonth(month, paramMap));
			modelMap.put("eventTypesList",paramMap.get("eventTypesList"));
		}
		modelMap.put("eventTypeMap2",StatisticsTaskGameEventTypeMapper.getRelateEventTypes(StatisticsTaskName.GAIN_PROP));
		modelMap.put("eventTypeMap",GameLogEventType.getEventTypeMap());
